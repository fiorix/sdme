...
Nested Docker has two requirements the defaults do not cover: a real filesystem for the container root so Docker's storage driver works, and permission to use the `bpf` syscall for the cgroup v2 device controller. sdme's `--storage btrfs` provides the first (see [using a different root filesystem](different-rootfs.md#storage-backends)); `--system-call-filter` provides the second. See also the [networking](networking.md) tutorial, which this one builds on.

> [!WARNING]
> **Why --storage btrfs and --system-call-filter**
>
...
Create and enter a container from an imported Ubuntu rootfs (see [using a different root filesystem](different-rootfs.md) to import one):
...
See the [architecture](../architecture.md) and [security](../security.md) documentation for how sdme's storage backends, capabilities, and seccomp filtering work.
