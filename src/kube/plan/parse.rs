...
fn warn_unknown_fields(raw: &serde_norway::Value, path: &str, known: &[&str]) {
    if let serde_norway::Value::Mapping(map) = raw {
        for (key, _val) in map {
...
fn warn_pod_spec_unknown_fields(spec_value: &serde_norway::Value) {
    warn_unknown_fields(spec_value, "spec", KNOWN_POD_SPEC_FIELDS);

    if let serde_norway::Value::Mapping(map) = spec_value {
        // Check securityContext fields.
        if let Some(sc) = map.get(serde_norway::Value::String("securityContext".into())) {
        ...
        for list_key in ["containers", "initContainers"] {
            if let Some(serde_norway::Value::Sequence(containers)) =
                map.get(serde_norway::Value::String(list_key.into()))
            {
                for (i, c) in containers.iter().enumerate() {
                    let fallback = format!("{i}");
                    let cname = c
                        .as_mapping()
                        .and_then(|m| m.get(serde_norway::Value::String("name".into())))
                        .and_then(|v| v.as_str())
                        .unwrap_or(&fallback);
                    warn_unknown_fields(
                        c,
                    ...
                    if let Some(csc) = c
                        .as_mapping()
                        .and_then(|m| m.get(serde_norway::Value::String("securityContext".into())))
                    {
                        warn_unknown_fields(
                            csc,
...
pub(crate) fn parse_yaml(content: &str) -> Result<(String, PodSpec)> {
    let manifest: KubeManifest =
        serde_norway::from_str(content).context("failed to parse Kubernetes YAML")?;
    ...
    match manifest.kind.as_str() {
        "Pod" => {
            ...
            let spec: PodSpec =
                serde_norway::from_value(spec_value).context("failed to parse Pod spec")?;
            Ok((name, spec))
        }
        "Deployment" => {
            ...
            if let serde_norway::Value::Mapping(ref map) = spec_value {
                if let Some(serde_norway::Value::Mapping(ref tmap)) =
                    map.get(serde_norway::Value::String("template".into()))
                {
                    if let Some(tspec) = tmap.get(serde_norway::Value::String("spec".into())) {
                        warn_pod_spec_unknown_fields(tspec);
                    }
                }
            }
            let deploy_spec: DeploymentSpec =
                serde_norway::from_value(spec_value).context("failed to parse Deployment spec")?;
