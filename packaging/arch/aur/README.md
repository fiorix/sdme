...
- **`package()` asserts the binary is over 8M.** A failed probe build is only a warning in `build.rs`, which then embeds an empty placeholder; without the check, a package whose kube probes silently do not work would look like a clean build.
