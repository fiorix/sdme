...
- **`package()` asserts the binary is over 8M.** `build.rs` fails the build outright if the probe cannot be embedded; the size check remains as a backstop so a probe-less binary (e.g. via `SDME_SKIP_PROBE`) can never look like a clean build.
