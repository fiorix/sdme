...
- **Multi-stage builds**: `sdme fs build` supports multiple `FROM` directives. Each `FROM <rootfs> AS <alias>` starts a new build stage. Later stages can use `COPY --from=<alias> <src> <dst>` (or `--from=<index>` for 0-based numeric index) to copy files from a prior stage's merged filesystem. Intermediate stages are captured to temp dirs under `$TMPDIR` and cleaned up after the build. Intermediate staging containers are named `build-<name>-s<index>`; the final stage uses `build-<name>`. The `src` path in `COPY --from=` is sanitized (leading `/` stripped, `..` rejected) to prevent escaping the stage's captured directory.
