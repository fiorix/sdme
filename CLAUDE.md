...
The probe binary (`sdme-kube-probe`) is built and embedded by `build.rs`. Override with `SDME_KUBE_PROBE_PATH` for cross-compilation. A failed probe build fails the whole build; set `SDME_SKIP_PROBE=1` for tests or builds without kube probe support.
