...
- **`Requires: systemd >= 255` in the spec is what keeps sdme off chroots that cannot run it.** sdme needs systemd >= 255; CentOS Stream 9 ships 252 and EPEL does not replace base packages, so an unversioned Requires would let the RPM install there and fail at runtime. Stream 10 ships 257 and is fine.
